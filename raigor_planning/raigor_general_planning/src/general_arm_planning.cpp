...
Manipulator::Manipulator()
{
    ...
    // sub_arm_state_ = n_.subscribe("/raigor_cartesian_velocity_controller/ee_state", 1,
...
bool Manipulator::plan_mode(ros::ServiceClient& controller_switch_)
{
    ...
    switcher.request.stop_controllers.push_back("raigor_cartesian_velocity_controller_sim");
...
bool Manipulator::custom_mode(ros::ServiceClient& controller_switch_)
{
    ...
    switcher.request.start_controllers.push_back("raigor_cartesian_velocity_controller_sim");
    switcher.request.strictness = 0;
    switcher.request.start_asap = false;
    switcher.request.timeout = 0.;
    controller_switch_.call(switcher);
...
void Manipulator::memory_servo()
{
    ...
    while(n_.ok())
    {
        ...
        // for the maze
        if (desired_ee_arm_position(0) > 0.61)
        {
            desired_ee_arm_position(0) = 0.61;
            // ROS_INFO_STREAM("too large x");
        }
        if (desired_ee_arm_position(0) < 0.24)
        {
            desired_ee_arm_position(0) = 0.24;
            // ROS_INFO_STREAM("too small x");
        }
        if (desired_ee_arm_position(1) > 0.65)
        {
            desired_ee_arm_position(1) = 0.65;
            // ROS_INFO_STREAM("too large y");
        }
        if (desired_ee_arm_position(1) < -0.65)
        {
            desired_ee_arm_position(1) = -0.65;
            // ROS_INFO_STREAM("too small y");
        }
        
        // for the eod 
        // if (desired_ee_arm_position(0) > 0.50)
        // {
        //     desired_ee_arm_position(0) = 0.50;
        //     ROS_INFO_STREAM("too large x");
        // }
        // if (desired_ee_arm_position(0) < 0.34)
        // {
        //     desired_ee_arm_position(0) = 0.34;
        //     ROS_INFO_STREAM("too small x");
        // }
        // if (desired_ee_arm_position(1) > 0.55)
        // {
        //     desired_ee_arm_position(1) = 0.55;
        //     ROS_INFO_STREAM("too large y");
        // }
        // if (desired_ee_arm_position(1) < -0.55)
        // {
        ...
        // general refine limitator
        // Eigen::Vector2f ee_lim_vec(desired_ee_arm_position(0), desired_ee_arm_position(1));
        // float ee_lim_dist = ee_lim_vec.norm();
        // float ratio;
        // if (ee_lim_dist > 0.65)
        // {
        //     ratio = 0.65 / ee_lim_dist;
        //     desired_ee_arm_position(0) = desired_ee_arm_position(0) * ratio;
        //     desired_ee_arm_position(1) = desired_ee_arm_position(1) * ratio;
        // }
        // if (ee_lim_dist < 0.30)
        // {
        //     ratio = 0.30 / ee_lim_dist;
        //     desired_ee_arm_position(0) = desired_ee_arm_position(0) * ratio;
        //     desired_ee_arm_position(1) = desired_ee_arm_position(1) * ratio;
        // }
...
void Manipulator::preset_servo()
{
    ...
    while(n_.ok() && controller_flag_ == 0)
    {
        ...
        ee_pose_msg.pose.position.z = 0.53;
...
{
    arm_command_time_ = desired_ee_global_pose.header.stamp.sec + 1e-9*desired_ee_global_pose.header.stamp.nsec;
    if (arm_command_time_ - last_arm_command_time_> 0.001)
    {
        last_arm_command_time_ = arm_command_time_;
        last_desired_ee_map_position_ = desired_ee_map_position_;
        diff_index_ = 0;
    }
